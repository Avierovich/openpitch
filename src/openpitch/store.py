...
def read_company(company_id: str) -> Company | None:
    raw = _read_json(data_dir() / "companies" / f"{company_id}.json")
...
def read_claims(company_id: str) -> list[Claim]:
    raw = _read_json(data_dir() / "claims" / f"{company_id}.json") or []
...
def read_history(company_id: str, metric: str) -> list[dict]:
    return _read_jsonl(data_dir() / "history" / company_id / f"{metric}.jsonl")
...
def read_events(since: str | None = None) -> list[dict]:
    rows = _read_jsonl(data_dir() / "events" / "feed.jsonl")
    if since:
...
def read_universe() -> dict | None:
    return _read_json(data_dir() / "universe.json")
